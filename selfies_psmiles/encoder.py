...
def _normalize_psmiles(smiles: str) -> str:
    """
    Minimal PSMILES normalization:
        Strip outer {...} or (...) n wrappers used for repeat units.
        Keep internal [*] termini; ignore explicit repeat counts (n, m-n).

    """
    s = smiles.strip()
    # remove single outermost braces: { ... } or ( ... )<count>
    # { ... }n -> ...
...
def encoder(smiles: str, strict: bool = True, attribute: bool = False) -> str:
    ...
    try:
        # accept PSMILES by normalizing common polymer wrappers
        norm = _normalize_psmiles(smiles)
        mol = smiles_to_mol(norm, attributable=attribute)
...
def _atom_to_selfies_psmiles(bond, atom):
    ...
    if getattr(atom, "element", None) == "*":
        ...
        content = atom_to_smiles(atom, brackets=False)
    return f"[{bond_char}{content}]"
