...
from .constants import ELEMENTS, INDEX_ALPHABET
...
_DEFAULT_CONSTRAINTS = {
    ...
    "P": 5, "P+1": 4, "P-1": 6,
    "S": 6, "S+1": 5, "S-1": 5,
    "*" : 1, # PSMILES dummy star: default valence 1
    "?": 8
}
    ...
    if isinstance(bond_constraints, str):
        ...
        for key, value in bond_constraints.items():
            ...
            if key == "?":
                ...
                valid = (key in ELEMENTS) or (key == "*")
            else:
                base = key[:j]
                valid = ((base in ELEMENTS) or (base == "*")) and key[j + 1:].isnumeric()
            if not valid:
