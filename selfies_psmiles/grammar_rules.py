...
def _process_atom_selfies_no_cache(symbol):
    ...
    # Special-case for polymer dummy/start atom[*]
    if element == "*":
        # treat as a non-aromatic dummy with default capacity from constraints
        atom_fac = functools.partial(Atom, element="*", is_aromatic=True,
                                     isotope=None, chirality=None,
                                     h_count=0, charge=0)
...
def _build_atom_cache():
    ...
    # also cache dummy star with common bond chars
    for bc in ["", "=", "#"]:
        cache[f"[{bc}*]"] = _process_atom_selfies_no_cache(f"[{bc}*]")
    return cache
